...
HashSet은 두 객체의 중복 여부를 체크할때 List 계열보다 엄격 Object 클래스는 equal() + hashCode()를 이용하여 그 값이 같은 경우 같은 객체로 인식한다.
...
두 가지 종류가 있는데, 하나는 문서를 한번 쭉 읽으면서 tag 의 발생별로 처리하는 방법( SAX )이 있고 , 또 하나는 문서를 다 읽고 난 후, 문서 구조 전체를 자료구조에 저장하여 탐색하면서 처리하는 방법( DOM )도 있습니다.

SAX 는 빠른 반면에 한번에 처리하기 때문에 다양한 탐색을 할 수 없습니다.

 DOM 방식은 다양한 탐색이 가능하지만 느리고, 무겁습니다.
...
-  nested 가 지켜져야한다. 한 태그가 열렸으면 그 태그가 닫치기 전에 그 부모가 닫쳐서는 안된다.
...
전송의 보장(준것을 다시 다 받을 수 있음) 순서 보장



### UDP

전송이 보장이 되지 않음. Broadcasting에서 많이 활용됨. 하지만 엄청빠름! ex) 라디오



### Concurrent

어떤 Job이 여러개 동시에 처리된다는 개념, 브라우저 여러 개를 띄워서 동시에 여러 Site를 접속하거나 엑셀 문서 여러 개 동시에 편집하는 등



### Parallel

Parallel 어떤 하나의 Job을 쪼개서 여러 Sub-job으로 나누고 이를 동시에 처리해서 완성하는 개념
...
에서 테이블 안의 데이터를 다 지우는 작업

Delete FROM TABLE => 로그를 남겨서 Rollback을 할 수 있음.

TRUNCATE TABLE => 로그를 남기지 않고 그냥 싸그리 없애버림 Rollback이 힘듬

DROP TABLE => 테이블 자체를 날림





abstract class 안에 모두다 구현한 메소드만 존재할 수도 있다 이것은 상속 강요를 위한 일이다.
